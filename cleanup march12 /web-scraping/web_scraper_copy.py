...
class FDA_Web_Scraper_2():
    ...
    def setup_for_gather_urls_for_letters(self):
        ...
        elems = self.driver.find_element(
            By.CLASS_NAME, "form-control.input-sm")
        self.driver.execute_script("arguments[0].scrollIntoView();", elems)
        elem = Select(self.driver.find_element(
            By.CLASS_NAME, "form-control.input-sm"))
    ...
    def merge_with_exported_data(self):
        df = pd.read_csv("data/warning-letters-exported.csv",
                         header=[0], on_bad_lines='skip', encoding="UTF-8")
        df2 = pd.DataFrame.from_dict(self.codes)
        df2['Company Name'] = df2['Company Name'].apply(
            lambda x: x.replace("\n", ""))
        df2['Company Name'] = df2['Company Name'].apply(lambda x: x.strip())
        df['Company Name'] = df['Company Name'].apply(lambda x: x.strip())

        merged = df.merge(
            df2, how='left', left_on='Company Name', right_on='Company Name')
    ...
    def scrape(self):
        ...
        self.filter_links()
        print("Succesfully gathered ", len(
            self.warning_letter_links), " URLs to scrape\n")
